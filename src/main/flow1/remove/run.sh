...
report="$log.report"
echo "When there are not errors reported, you can safely remove the folder and it's content: ${fileSet}" > $report
groovy $global_home/remove.file.groovy -file "$file_instruction" -access_token $flow_access_token -or $or >> $report
